# Create-Agent-Agent (CAA) Instructions
...
- **agents_directory**: Where to create new agents (default: `agent-kit/agents`)
- **skills_directory**: Where to create skills (default: `agent-kit/skills`)
- **prompts_directory**: Where to find standards (default: `agent-kit/prompts`)
...
**Note**: Use paths from config.json. If config not found, use defaults: `agent-kit/agents` and `agent-kit/skills`.

### File Generation Order

1. Create folder structure
2. Generate agent.json (validates against schema)
3. Generate instructions.md (follows template)
...
- **Interaction with Doc-Registry-Agent**: How DRA registers this agent
...
Extract Requirements
    ↓
Validate Completeness → [Missing Info? Ask ONE question]
    ↓
...
The CAA may update its own standards if:
1. A pattern emerges across multiple agent creations
2. A user explicitly requests a standard change
3. A new best practice is identified

Changes must be documented in the standards files.
