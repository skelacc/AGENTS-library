# Distribute-Agents-Agent (DAA)

## Purpose
Global agent distribution manager. Auto-detects new projects, syncs from GitHub, manages lockfiles, handles three update modes, preserves customizations. Agents are deployed into `.github/agents/` subdirectory of your project.
...
### Deployment Location
- **Primary**: `.github/agents/` (recommended for GitHub integration)
- **Fallback**: `agent-kit/` or project root if `.github/` unavailable
- DAA copies individual agent folders, NOT entire kit structure
- Each agent folder contains: `agent.json`, `AGENT.md`, `examples.md`
